...
Game.Launch = function () {
	...
	Game.Init = function () {
		...
		Game.GrabData = function () {
			if (!App) ajax('grab.json', Game.GrabDataResponse);
		...
		Game.ServerLoad = async function () {
			const url = localStorageGet('saveUrl')
			if (!url) {
				return Game.Notify(loc('URLが指定されていません'), '')
			}
		...
		Game.UpdateMenu = function () {
			...
			if (Game.onMenu == 'prefs') {
				...
				str +=
					...
					'<div class="listing"><a class="option smallFancyButton" ' + Game.clickStr + '="Game.showUrlPrompt();PlaySound(\'sounds/tick.mp3\');">' + loc("サーバーのURLを指定する") + '</a><div class="listing"><a class="option smallFancyButton" ' + Game.clickStr + '="Game.ServerLoad();PlaySound(\'sounds/tick.mp3\');">' + loc("サーバーから読み込む") + '</a><label>' + loc("指定されたURLからデータを読み込みます") + '</label></div>' +
...
window.onload = function () {
	if (!Game.ready) {
		var loadLangAndLaunch = function (lang, firstLaunch) {
			...
			LoadLang('./js/locales/EN.js?v=' + Game.version, function (lang) {
				return function () {
					locStringsFallback = locStrings;
					LoadLang('./js/locales/' + lang + '.js?v=' + Game.version, function () {
						var launch = function () {
							Game.Launch();
